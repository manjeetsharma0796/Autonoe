...
/** Pick badge class from symbol. Keeps the WOKB/BTC/ETH colours, generic gold for the rest. */
  ...
  return (
    <div ref={root} className={`${styles.mk} ${styles.reveal}`}>
      ...
      <div className={styles.mfoot}>
        <span className="ping" /> Live public spot prices ·
        refreshes every ~15s · on-chain execution on the testnet terminal only
      </div>
    </div>
  );
}
...
function MarketRow({
  ...
  const slug = token.symbol === "WOKB" ? "mUSD-WOKB" : `mUSD-${token.symbol}`;
