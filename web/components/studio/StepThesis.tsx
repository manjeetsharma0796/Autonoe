...
const POPULAR_SYMBOLS = ["WOKB", "BTC", "ETH", "SOL", "SUI", "BNB", "XRP", "DOGE"];
...
function TokenPicker({ value, onChange }: TokenPickerProps) {
  ...
  const load = useCallback(async (q: string) => {
    ...
    try {
      ...
      if (!q) {
        // Reorder: WOKB first, then popular symbols, then rest by volume.
...
export function StepThesis({ onSendToJudge }: StepThesisProps) {
  ...
  const [suggestedPair, setSuggestedPair] = useState("WOKB");
              ...
              <div className={styles.field} style={{ marginTop: 18 }}>
                ...
                {suggestedPair !== "WOKB" && (
                  <p style={{ marginTop: 6, fontSize: 12, color: "var(--muted)", fontFamily: "var(--mono)" }}>
                    Advise-only · only WOKB is executable on-chain
                  </p>
                )}
              </div>
              <div className={styles.field}>
