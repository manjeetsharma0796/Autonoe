...
const Option = z.object({
  ...
  asset: z.string().describe('one of: WOKB, BTC, ETH, SUI, SOL'),
...
const ThesisCore = z.object({
  suggestedPair: z.string().describe('one of: WOKB, BTC, ETH, SUI, SOL'),
...
const ASSETS = ['WOKB', 'BTC', 'ETH', 'SUI', 'SOL'] as const;
...
function normAsset(v: unknown): Asset {
  ...
  return ASSETS.find((a) => s.includes(a)) ?? 'WOKB';
}
...
const SYSTEM =
  ...
  'Assets tradable against the mUSD stablecoin: WOKB (real AMM), BTC, ETH, SUI, SOL (synthetics). ' +
    ...
    .invoke([
      ...
      new HumanMessage(
        'Now output the final thesis as structured data, grounded strictly in the tool evidence above. ' +
          ...
          'risk must be exactly "low", "medium", or "high"; asset one of WOKB, BTC, ETH, SUI, SOL.',
      ),
    ]);
