...
// Hybrid routing: WOKB settles via the AMM; every other asset is an oracle-priced
...
export interface ExecuteOptionInput {
  ...
  privateKey: Hex;
  direction: Direction;
  asset: string; // 'WOKB' (AMM) | 'BTC' | 'ETH' | 'SUI' | 'SOL' (synthetic)
...
export interface ExecuteResult {
  ...
  explorerUrl: string;
  amountInMUSD: number;
  /** AMM: WOKB out (18dec base units). Synthetic: collateral (mUSD base units). */
  amountOut: string;
  positionId?: string;
...
/** Execute a chosen option: real AMM swap (WOKB) or open a synthetic (others). */
